...
def compute_feature_distances(features1, features2):
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    ans = np.zeros((features1.shape[0], features2.shape[0]))
    for i,feat in enumerate(features1):
        ans[i,:] = np.linalg.norm(features2 - feat[np.newaxis, :], axis=1)
    dists = ans
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    for i, temp in enumerate(features1):
        dists = distances[i,:]
        sorted_dists = np.argsort(dists)
        if dists[sorted_dists[0]] / dists[sorted_dists[1]] < 0.85:
            matches.append([i, sorted_dists[0]])
            confidences.append(dists[sorted_dists[0]])
    matches = np.asarray(matches, dtype=np.int)
    confidences = np.asarray(confidences)
    confidences /= np.max(confidences)
    sorted_dists = np.argsort(-confidences)
    matches = matches[sorted_dists]
    confidences = confidences[sorted_dists]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
